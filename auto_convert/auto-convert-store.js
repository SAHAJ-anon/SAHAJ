...
function distributeFunctions() {
	...
	for (let i = 0; i < ast.children.length; i++) {
		...
		let contractName = path.basename(filepath).replace('.sol', '');
		// console.log(`Found ${node.type} which is ${node.kind}`)
		if(node.type == "ContractDefinition" && node.kind != "interface" && node.kind != "library") {
			numContracts += 1;
		}
		if (node.type === "ContractDefinition" && node.kind === "contract") {
	...
	if(numContracts >= 2) {
		console.log(`${numContracts} contracts found`)
		throw "More than one contracts found";
	}
