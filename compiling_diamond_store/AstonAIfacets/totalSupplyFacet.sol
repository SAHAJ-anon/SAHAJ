...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        ds.astonFee.transfer(ethForMarketing);
    }
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (ds._isExcludedFromAstonFee[from]) {
            ds._astonTax = 0;
            _basicTransfer(from, to, amount, false);
            return;
        } else {
            require(ds.launch, "Wait till Aston Launch!");
            ...
            if (block.number < ds.launchedAt + ds.launchDelay) {
                ...
                if (from == ds.uniswapV2Pair) {
                    ...
                    ds._astonTax = ds.buyTax;
                } else if (to == ds.uniswapV2Pair) {
                    uint256 tokensToSwap = balanceOf(address(this));
                    if (tokensToSwap > minAstonSwap && !ds.inSwapAndLiquify) {
                    ...
                    ds._astonTax = ds.sellTax;
                } else {
                    ds._astonTax = ds.transferTax;
                }
            }
        }

        uint256 taxTokens = (amount * ds._astonTax) / 100;
