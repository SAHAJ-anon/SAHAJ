...
contract totalSupplyFacet is IERC20, Ownable {
    ...
    function createAxiswapPairs() external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(!ds.launch, "Already Axiswap AI launched!");
    ...
    function changeAxiswapThreshold(uint256 amount) external onlyOwner {
        ...
        emit updateAxiswapThresold(ds.swapThreshold);
    }
    ...
    function _transfer(
        ...
        if (ds._noAxiswapFee[from] || ds._noAxiswapFee[to]) {
            takeFee = false;
        } else {
            require(ds.launch, "Trading is not opened!");

            if (is_AxiswapSell(from, to) && !ds.inSwap) {
        ...
        uint256 amountAfterFee = (takeFee)
            ? takeAxiswapTaxes(from, is_AxiswapSell(from, to), amount)
            : amount;
    ...
    function isNoAxiswapInternalFees(address ins) internal view returns (bool) {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        return ds._noAxiswapFee[ins] && ins != owner() && ins != address(this);
    }
    ...
    function internalSwap(
        ...
        if (contractBalance > ds.swapThreshold) {
            ...
            ds.axiswapFees.transfer(ethForMarketing);
        }
    }
    function takeAxiswapTaxes(
        address from,
        bool issell,
        uint256 amount
