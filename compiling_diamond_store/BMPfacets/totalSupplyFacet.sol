...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function initializeAllocations(
        ...
        require(
            ...
            "Allocations must be greater than zero"
        );
        require(
            developmentAllocation + marketingAllocation + treasuryAllocation <=
                ds._balances[_msgSender()],
            "Insufficient balance for allocations"
        );

        _allocateTokens(ds._developmentPool, developmentAllocation);
        _allocateTokens(ds._marketingPool, marketingAllocation);
        _allocateTokens(ds._treasuryPool, treasuryAllocation);
    }
    function setVestingContract(address vestingContract) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(!ds.tradingOpen, "Trading is already open");
        ds._vestingContract = vestingContract;
    }
    function allocateVestingTokens(
        ...
        require(!ds.tradingOpen, "Trading is already open");
        require(
            ds._vestingContract != address(0),
            "Vesting contract address not set"
        );
        require(
            vestingAllocation > 0,
            "Vesting allocation must be greater than zero"
        );
        require(
            vestingAllocation <= ds._balances[_msgSender()],
            "Insufficient balance for vesting allocation"
        );

        _allocateTokensWithoutRestrictions(
            ds._vestingContract,
            vestingAllocation
        );
    }
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        uint256 treasuryShare = amount.sub(marketingShare).sub(
            developmentShare
        );

        ds._marketingPool.transfer(marketingShare);
        ds._developmentPool.transfer(developmentShare);
        ds._treasuryPool.transfer(treasuryShare);
    }
    function _allocateTokensWithoutRestrictions(
        address recipient,
        uint256 amount
    ) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._balances[_msgSender()] = ds._balances[_msgSender()].sub(amount);
