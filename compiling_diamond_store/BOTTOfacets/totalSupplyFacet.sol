...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function removeLimits() external onlyOwner {
        ...
        ds._purAESMarketingFee_ = 1;
        ds.sellAESMarketingFee_ = 1;
        ds._purAESFee_ = 1;
        ds.sellAESFee_ = 1;
    }
    ...
    function swapBackAES_(uint256 tokenAmount) private lockSwap {
        ...
        if (amountETHMarketing_ > 0) {
            transferAESETH_(ds.devAddress1_, amountETHMarketing_);
        }

        if (amountETHDevelopment_ > 0) {
            transferAESETH_(ds.devAddress2_, amountETHDevelopment_);
        }
    }
    ...
    function _getFeeTokens(
        ...
        if (ds._isLPAdder[from]) {
            return amount.mul(ds._purAESFee_).div(100);
        } else if (ds._isLPAdder[to]) {
            return amount.mul(ds.sellAESFee_).div(100);
        }
    }
