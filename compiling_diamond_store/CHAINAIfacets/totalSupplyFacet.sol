...
contract totalSupplyFacet is IERC20, Context {
    ...
    function changeBuySellFee(
        ...
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(buyFee <= 5, "Tax too high");
        require(sellFee <= 5, "Tax too high");
        ds._buyTax = buyFee;
        ds._sellTax = sellFee;
    }
    function changeTransferFee(uint256 trFee) public onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(trFee <= 5, "Tax too high");
        ds._transferTax = trFee;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!ds._isExcludedFromFee[from] && !ds._isExcludedFromFee[to]) {
            if (to != ds.uniswapV2Pair) {
                require(
                    balanceOf(to) + amount <= ds.maxWalletLimit,
                    "Exceeds the ds.maxWalletLimit."
                );
            }
            ...
            if (to != ds.uniswapV2Pair && from != ds.uniswapV2Pair) {
                taxAmount = amount.mul(ds._transferTax).div(100);
            }
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        (bool callSuccess, ) = payable(ds._taxWallet).call{value: amount}("");
