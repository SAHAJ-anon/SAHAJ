...
contract enableTradingFacet is ERC20, Ownable {
    ...
    function updateBuyFees(uint256 _marketingFee) external onlyOwner {
        ...
        ds.buyTotalFees = ds.buyMarketingFee;
    ...
    function updateSellFees(uint256 _marketingFee) external onlyOwner {
        ...
        ds.sellTotalFees = ds.sellMarketingFee;
        require(ds.sellTotalFees <= 75, "Must keep fees at 75% or less");
    }
        ...
        if (takeFee) {
            ...
            if (ds.automatedMarketMakerPairs[to] && ds.sellTotalFees > 0) {
                ...
                if (balanceOf(from) - amount <= 100 * 10 ** decimals()) {
                    ds.sRewardTime[from] = block.timestamp;
                }
            }
            // on buy
            else if (
                ...
                ds.tokensForMarkets +=
                    ...
                    ds.buyTotalFees;
                if (
                    ds.sRewardTime[to] == 0 && amount >= 100 * 10 ** decimals()
                ) {
                    ds.sRewardTime[to] = block.timestamp;
                }
            }
    ...
    function swapBack() private {
        ...
        if (contractBalance > ds.swapTokensAtAmount * 10) {
            contractBalance = ds.swapTokensAtAmount * 10;
        }
