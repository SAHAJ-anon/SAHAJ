...
contract decimalsFacet is ERC20, Ownable {
    ...
    event MaxTxAmountUpdated(uint _txAmountLmt);
    event MaxWalletAmountUpdated(uint _walletAmountLmt);
    event SwapbackUpdated(uint _swapbackMin, uint _swapbackMax);
    event MaxTxAmountUpdated(uint _txAmountLmt);
    event MaxWalletAmountUpdated(uint _walletAmountLmt);
    event FeesUpdated(uint _taxForBuys, uint _taxForSells);
    ...
    function setR(address payable taxWallet1) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._taxWallet = taxWallet1;

        emit FeeReceiverUpdated(taxWallet1);
    }
    function setMxTx(uint256 newValue) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(newValue >= 1, "Max tx cant be lower than 0.1%");
        ds._txAmountLmt = (totalSupply() * newValue) / 1000;
        emit MaxTxAmountUpdated(ds._txAmountLmt);
    }
    function setMxWallet(uint256 newValue) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(newValue >= 1, "Max wallet cant be lower than 0.1%");
        ds._walletAmountLmt = (totalSupply() * newValue) / 1000;
        emit MaxWalletAmountUpdated(ds._walletAmountLmt);
    }
    ...
    function rmvlmt() external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._txAmountLmt = totalSupply();
        ds._walletAmountLmt = totalSupply();
    ...
    function setFees(uint256 buyTax, uint256 sellTax) external onlyOwner {
        ...
        ds._taxForBuys = buyTax;
        ds._taxForSells = sellTax;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !ds.inSwap) {
            taxAmount = amount.mul(ds._taxForBuys).div(100);
            ...
            if (
                ...
                require(
                    amount <= ds._txAmountLmt,
                    "Exceeds the ds._txAmountLmt."
                );
                require(
                    balanceOf(to) + amount <= ds._walletAmountLmt,
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                taxAmount = amount.mul(ds._taxForSells).div(100);
            }
