...
contract updateMaxBuyAmountFacet is ERC20, Ownable {
    ...
    function setRewardsAddress(address _RewardsAddress) external onlyOwner {
        ...
        require(
            ...
            "_RewardsAddress address cannot be 0"
        );
        ds.RewardsAddress = payable(_RewardsAddress);
    ...
    function enableTrading(
        ...
        ds.tradingActive = _status;
        ds.swapEnabled = true;
        emit EnabledTrading(ds.tradingActive, _deadBlocks);

        if (ds.tradingActive && ds.tradingActiveBlock == 0) {
            ds.tradingActiveBlock = block.number;
            ds.deadBlocks = _deadBlocks;
        }
    }
    ...
    function swapBack() private {
        ...
        (success, ) = address(ds.RewardsAddress).call{value: ethForRewards}("");
