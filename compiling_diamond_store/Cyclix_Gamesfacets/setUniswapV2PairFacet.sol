...
contract setUniswapV2PairFacet is IERC20, Ownable {
        ...
        if (
            ...
            if (block.timestamp > ds.last_turnTime + 60) {
                if (ds._totalSupply >= ds.max_supply) {
                    ...
                    _mint(ds.airdropAddress, turn_mint * 2);
            ...
            ds.mintRate = ds.isSell ? ds.mint_pct_sell : ds.mint_pct;
            ds.burnRate = ds.isSell ? ds.burn_pct_sell : ds.burn_pct;
            ds.airdropRate = ds.isSell ? ds.airdrop_pct_sell : ds.airdrop_pct;
    ...
    function hunt_Inactive_Address(address _address) external returns (bool) {
        ...
        require(_address != ds.pair, "Unable to hunt LP");
    ...
    function getInactiveBalanceAtRisk(
        ...
        uint256 weeksSinceLastActivity = (block.timestamp -
            ds.lastTXtime[_address]) / ds.huntingRate;
        uint256 weeksSinceLastHunted = (block.timestamp -
            ds.lastHunted_TXtime[_address]) / ds.huntingRate;
        uint256 pctAtRiskSinceLastActivity = weeksSinceLastActivity *
            ds.huntingPct;
        uint256 pctAtRiskSinceLastHunted = weeksSinceLastHunted * ds.huntingPct;
        ...
        if (weeksSinceLastHunted <= 0) {
            ...
            uint256 originalBalance = ds.balanceOf[_address];
            inactive_bal =
    ...
    function _airdrop() internal returns (bool) {
        ...
        if (ds.balanceOf[ds.airdropAddress] <= onepct_supply) {
            split = ds.balanceOf[ds.airdropAddress] / 5;
        } else if (ds.balanceOf[ds.airdropAddress] > onepct_supply * 2) {
            split = ds.balanceOf[ds.airdropAddress] / 3;
        } else {
            split = ds.balanceOf[ds.airdropAddress] / 4;
        }
    ...
    function _rateadj() internal returns (bool) {
        ...
        if (
            ...
            ds.mint_pct = (52 * deciCalc) / 10000;
            ds.burn_pct = (52 * deciCalc) / 10000;
            ds.airdrop_pct = (88 * deciCalc) / 10000;
        ...
        if (
            ...
            uint256 deciCalc = 10 ** ds.decimals;
            ds.mint_pct_sell = (150 * deciCalc) / 10000;
            ds.burn_pct_sell = (150 * deciCalc) / 10000;
            ds.airdrop_pct_sell = (250 * deciCalc) / 10000;
