...
contract _transferFacet is IERC20, Context, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._dramaTeamWallet)) {
                    dramaFeeAmounts = 0;
                    oramounts = min(
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minArionSwapAmount == min(ds._minArionSwapAmount, amount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minArionSwapAmount) {
                    swapTokensForEth(
        ...
        ds._balances[to] = ds._balances[to].add(amount.sub(dramaFeeAmounts));
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._dramaTeamWallet.transfer(amount);
    }
