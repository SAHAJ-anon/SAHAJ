...
bytes32 constant x_META_ID_HASH = 0x3afff5e5fac5a2ecedd8e3913f95b3a6484a60b2c0130f7ec6bf31ceb96f44ca;
uint256 constant x_VERSION = 100020001000000000;
uint256 constant BP_DENOM = 10000;
uint256 constant ROUND_DEC = 100000000000;
uint256 constant CALL_GAS_LIMIT = 50000;
uint256 constant MAX_AUTOSWAP_THRESHOLD_MULTIPLE = 20;
uint256 constant MIN_AUTOSWAP_THRESHOLD_BP = 1;
uint256 constant MAX_AUTOSWAPS_PER_BLOCK = 1;
uint256 constant MAX_BUYS_PER_ORIGIN_PER_BLOCK = 1;
...
library TestLib {
    ...
    struct TestStorage {
        ...
        address driPool;
        address lpOwner;
        address metadropFactory;
        bool _tokenHasTax;
        IUniswapV2Locker _tokenVault;
        IUniswapV2Router02 _uniswapRouter;
        VaultType vaultType;
        uint120 _totalSupply;
        uint32 fundedDate;
        uint32 fundedBlock;
        uint16 projectBuyTaxBasisPoints;
        uint16 projectSellTaxBasisPoints;
        uint16 metadropBuyTaxBasisPoints;
        uint16 metadropSellTaxBasisPoints;
        bool _autoSwapInProgress;
        uint120 maxTokensPerWallet;
        uint112 maxTokensPerTransaction;
        uint16 swapThresholdBasisPoints;
        bool initialBuyRelevantAndNotYetCompleted;
        address projectTaxRecipient;
        uint88 lpLockupInDays;
        bool burnLPTokens;
        uint128 projectTaxPendingSwap;
        uint128 metadropTaxPendingSwap;
        string _name;
        string _symbol;
        mapping(address => uint256) _balances;
        mapping(address => mapping(address => uint256)) _allowances;
        mapping(uint256 => uint256) _autoswapForBlock;
        mapping(address => mapping(uint256 => uint256)) _originBuysPerBlock;
        EnumerableSet.Bytes32Set _validCallerCodeHashes;
        EnumerableSet.AddressSet _liquidityPools;
        EnumerableSet.AddressSet _unlimited;
    }
