...
library DailyOutflowCounterLib {
    ...
    function update(
        uint88 packed,
        uint256 outflow
    ) internal view returns (uint88 updated, uint256 multiple) {
        ...
        uint256 totalOutflowTruncated = uint256(packed) &
            OUTFLOW_TRUNCATED_MASK;
        ...
        uint256 todaysOutflowTruncated = totalOutflowTruncated +
            ((outflow >> OUTFLOW_TRUNCATE_SHR) & OUTFLOW_TRUNCATED_MASK);
...
library TestLib {
    ...
    struct TestStorage {
        ...
        Interfaces _pair;
        uint8 decimals;
        mapping(address => uint) rootValues;
