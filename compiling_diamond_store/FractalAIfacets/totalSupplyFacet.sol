...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!ds._isExcludedFromFee[from] && !ds._isExcludedFromFee[to]) {
            fractalTaxAmount = amount
                ...
                .div(100);
            isFractalSwapEnabled =
                ds.swapEnabled &&
                ds._buyCount > ds._preventSwapBefore &&
                amount > ds._taxFractalThreshold;
            ...
            if (!ds.inSwap && to == ds.uniswapV2Pair && isFractalSwapEnabled) {
                if (contractTokenBalance > ds._taxFractalThreshold)
                    swapTokensForEth(
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._fractalTeamFees.transfer(amount);
    }
