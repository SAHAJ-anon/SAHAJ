...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            taxAmount = amount.mul(ds._tax).div(10000);

            //Anti Sniper Rule
            if (ds.antiSniperEnabled && from != address(this)) {
                require(ds.antisniper[to], "Failed to snipe");
            }

            if (from == ds.uniswapV2Pair && to != address(ds.uniswapV2Router)) {
                ds._buyCount++;

                // Disable ds.antisniper & tax
                if (ds._buyCount >= _antiSniperCount && ds.antiSniperEnabled) {
                    ds.antiSniperEnabled = false;
                    ds._tax = _tier1;
                }
            }
            if (to == ds.uniswapV2Pair && from != address(this)) {
                ...
                // Reduce Tax
                if (ds._buyCount >= _reductingPeriod1 && ds._tax == _tier1) {
                    ds._tax = _tier2;
                }
                if (ds._buyCount >= _reductingPeriod2 && ds._tax == _tier2) {
                    ds._tax = _tier3;
                }
                if (
                    block.timestamp >=
                    ds._tradingOpened.add(_reductingPeriod3) &&
                    ds._tax == _tier3
                ) {
                    ds._tax = _tier4;
                }
