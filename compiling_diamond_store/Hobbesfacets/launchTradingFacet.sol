...
import "./TestLib.sol";
contract launchTradingFacet is ERC20, Ownable {
    ...
    function setfeesForBuying(
        ...
        ds.totalBuyFee = ds.buyMktFee + ds.buyDevFee;
        require(
            ds.totalBuyFee <= 100,
            "Total buy fee cannot be higher than 100%"
        );
        emit BuyFeeUpdated(ds.totalBuyFee, ds.buyMktFee, ds.buyDevFee);
    }
    function setFeesForSelling(
        ...
        ds.totalSellFee = ds.sellMktFee + ds.sellDevFee;
        require(
            ds.totalSellFee <= 100,
            "Total sell fee cannot be higher than 100%"
        );
        emit SellFeeUpdated(ds.totalSellFee, ds.sellMktFee, ds.sellDevFee);
    }
    function excludeAddressFromFees(
        address account,
        bool excluded
    ) public onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
    ...
    function setAsDEXPair(address pair, bool value) public onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(
            pair != ds.dexPair,
