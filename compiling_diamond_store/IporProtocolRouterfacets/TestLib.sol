...
interface IAmmPoolsServiceStEth {
    ...
    function provideLiquidityStEth(
        address beneficiary,
        uint256 stEthAmount
    ) external payable;
    ...
    function redeemFromAmmPoolStEth(
        address beneficiary,
...
interface IAmmPoolsServiceWstEth {
    function provideLiquidityWstEth(
        address beneficiary,
        uint256 stEthAmount
    ) external payable;

    function redeemFromAmmPoolWstEth(
        address beneficiary,
        uint256 ipTokenAmount
    ) external;
...
interface IAmmPoolsService {
    ...
    function redeemFromAmmPoolUsdt(
        address beneficiary,
        uint256 ipTokenAmount
    ) external;
    ...
    function redeemFromAmmPoolUsdc(
        address beneficiary,
        uint256 ipTokenAmount
    ) external;
    ...
    /// @param asset Address of the asset.
    function rebalanceBetweenAmmTreasuryAndAssetManagement(
        address asset
    ) external;
}
...
interface IAssetManagementLens {
    ...
    function getAssetManagementConfiguration(
        address asset
    ) external view returns (AssetManagementConfiguration memory);
    ...
    function balanceOfAmmTreasuryInAssetManagement(
        address asset
...
interface IPowerTokenFlowsService {
    ...
    function claimRewardsFromLiquidityMining(
        address[] calldata lpTokens
    ) external;
...
interface IPowerTokenLens {
    ...
    function getPwTokensInCooldown(
        address account
    ) external view returns (PwTokenCooldown memory);
...
interface IAmmPoolsLens {
    ...
    function getAmmPoolsLensConfiguration(
        address asset
    ) external view returns (AmmPoolsLensPoolConfiguration memory);
    ...
    function getIpTokenExchangeRate(
        address asset
    ) external view returns (uint256);
    ...
    function getAmmBalance(
        address asset
    ) external view returns (IporTypes.AmmBalancesMemory memory);
}
...
library PauseManager {
    ...
    function isPauseGuardian(address account) internal view returns (bool) {
        mapping(address => bool) storage pauseGuardians = StorageLib
            .getPauseGuardianStorage();
...
library OwnerManager {
    ...
    function appointToOwnership(address newAppointedOwner) internal {
        ...
        StorageLib.AppointedOwnerStorage
            storage appointedOwnerStorage = StorageLib.getAppointedOwner();
...
interface IAmmSwapsLens {
    ...
    function getSwapLensPoolConfiguration(
        address asset
    ) external view returns (SwapLensPoolConfiguration memory);
    ...
    function getPnlPayFixed(
        address asset,
        uint256 swapId
    ) external view returns (int256 pnlValue);
...
contract AccessControl {
    ...
    modifier onlyAppointedOwner() {
        require(
            StorageLib.getAppointedOwner().appointedOwner == msg.sender,
    ...
    modifier onlyPauseGuardian() {
        require(
            ...
            IporErrors.CALLER_NOT_PAUSE_GUARDIAN
        );
        _;
    }
    ...
    function removePauseGuardians(
        address[] calldata guardians
    ) external onlyOwner {
        PauseManager.removePauseGuardians(guardians);
    }

    function _checkFunctionSigAndIsNotPause(
        bytes4 functionSig,
        bytes4 expectedSig
    ) internal view returns (bool) {
        if (functionSig == expectedSig) {
            require(
                StorageLib.getRouterFunctionPaused().value[functionSig] == 0,
                IporErrors.METHOD_PAUSED
            );
            return true;
        }
        return false;
    ...
    function _onlyOwner() internal view {
        require(
            StorageLib.getOwner().owner == msg.sender,
            IporErrors.CALLER_NOT_OWNER
        );
    }
...
abstract contract IporProtocolRouterAbstract is
    UUPSUpgradeable,
    AccessControl,
    IProxyImplementation
{
    ...
    function batchExecutor(
        ...
        for (uint256 i; i != length; ) {
            implementation = _getRouterImplementation(
                bytes4(calls[i][:4]),
                BATCH_OPERATION
            );
    ...
    function _getRouterImplementation(
        bytes4 sig,
        uint256 batchOperation
