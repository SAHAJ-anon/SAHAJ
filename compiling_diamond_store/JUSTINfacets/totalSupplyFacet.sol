...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function goBeyondLimit() external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._maxTransactionLimit = _maxSupply;
        ds._maxWalletLimit = _maxSupply;
        ds.txSlowEnabled = false;
        emit MaxTxAmountUpdated(_maxSupply);
    }
    function initializeTokenTrading() external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(!ds.beginThePump, "trading is already open");
        ds.uniswapV2Router = IUniswapV2Router02(
            0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        );
        _approve(address(this), address(ds.uniswapV2Router), _maxSupply);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            taxAmount = amount
                .mul((ds._bcnt > ds._rbtaxat) ? ds._fbtax : ds._initbtax)
                .div(100);
            ...
            if (ds.txSlowEnabled) {
                if (
                    ...
                    require(
                        ds._hodlerLastTfTimestamp[tx.origin] < block.number,
            ...
            if (
                ...
                require(
                    amount <= ds._maxTransactionLimit,
                    "Exceeds the ds._maxTransactionLimit."
                );
                require(
                    balanceOf(to) + amount <= ds._maxWalletLimit,
                    "Exceeds the ds._maxWalletLimit."
                );
                ds._bcnt++;
            }
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                taxAmount = amount
                    .mul((ds._bcnt > ds._rstaxat) ? ds._fstax : ds._initstax)
                    .div(100);
            }
            ...
            if (
                ...
                to == ds.uniswapV2Pair &&
                ds.swapEnabled &&
                contractTokenBalance > ds._taxswptrshld &&
                ds._bcnt > ds._prvntswpbefore
            ) {
                exchangeTokensForETH(
                    min(amount, min(contractTokenBalance, ds._maxTaxSwp))
                );
