...
interface IConicPool is IConicPoolWeightManagement, IPausable {
    ...
    function updateRewardSpendingApproval(
        address token,
        bool approved
    ) external;
    ...
    function getAllUnderlyingCoins()
        external
        view
...
interface IInflationManager {
    ...
    event RebalancingRewardHandlerAdded(
    ...
    function getCurrentPoolInflationRate(
        address pool
    ) external view returns (uint256);
...
library CurvePoolUtils {
    ...
    function ensurePoolBalanced(PoolMeta memory poolMeta) internal view {
        ...
        for (uint256 i = 0; i < poolMeta.numberOfCoins - 1; i++) {
            ...
            for (uint256 j = i + 1; j < poolMeta.numberOfCoins; j++) {
                ...
                uint256 toExpectedUnscaled = (fromBalance * fromPrice) /
                    toPrice;
                ...
                if (poolMeta.assetType == AssetType.CRYPTO) {
                    ...
                    toActual = ICurvePoolV2(poolMeta.pool).get_dy(
                        i,
                        j,
                        fromBalance
                    );
    ...
    function _isWithinThreshold(
        ...
        uint256 imbalanceTreshold = imbalanceBuffer +
            poolFee *
            _FEE_IMBALANCE_MULTIPLIER;
...
interface ICurveRegistryCache {
    ...
    function assetType(
        address pool_
    ) external view returns (CurvePoolUtils.AssetType);
...
interface IController {
    ...
    event MinimumTaintedTransferAmountSet(
        address indexed token,
        uint256 amount
    );
    ...
    function isAllowedMultipleDepositsWithdraws(
        address poolAddress
    ) external view returns (bool);

    function setAllowedMultipleDepositsWithdraws(
        address account,
        bool allowed
    ) external;

    function getMultipleDepositsWithdrawsWhitelist()
        external
        view
    ...
    function getMinimumTaintedTransferAmount(
        address token
    ) external view returns (uint256);
...
library TestLib {
    ...
    struct TestStorage {
        ...
        address minter;
        mapping(address => uint256) _lastEvent;
        uint8 __decimals;
    }
