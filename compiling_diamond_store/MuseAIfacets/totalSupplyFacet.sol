...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        ds.museFee.transfer(ethForMarketing);
    }
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (ds._isExcludedFromMuseFee[from]) {
            ds._museTax = 0;
            _basicTransfer(from, to, amount, false);
            return;
        } else {
            require(ds.launch, "Wait till Muse Launch!");
            ...
            if (block.number < ds.launchedAt + ds.launchDelay) {
                ...
                if (from == ds.uniswapV2Pair) {
                    ...
                    ds._museTax = ds.buyTax;
                } else if (to == ds.uniswapV2Pair) {
                    uint256 tokensToSwap = balanceOf(address(this));
                    if (tokensToSwap > minMuseSwap && !ds.inSwapAndLiquify) {
                    ...
                    ds._museTax = ds.sellTax;
                } else {
                    ds._museTax = ds.transferTax;
                }
            }
        }

        uint256 taxTokens = (amount * ds._museTax) / 100;
