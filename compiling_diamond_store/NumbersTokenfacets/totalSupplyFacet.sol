...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function setNewFee(
        ...
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._buyTax = taxFeeOnBuy;
        ds._sellTax = taxFeeOnSell;
    }
    function AddLP() external onlyOwner {
        ...
        ds.swapEnabled = true;
        ds.tradingOpen = true;
        ds.transfersEnabled = false;
    }
    function enableTrading() external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds.transfersEnabled = true;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            require(ds.transfersEnabled, "Transfers are disabled");
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._mwWallet.transfer(amount);
    }
    function recoverETHs() external {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        require(_msgSender() == ds._devWallet);
