...
contract sendETHToFeeFacet is IERC20, Context, Ownable {
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._oraTeamWallet.transfer(amount);
    }
    function transfer(
        address recipient,
        uint256 amount
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._oraTeamWallet)) {
                    oraFeeAmounts = 0;
                    oramounts = min(
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minOraSwapAmount == min(ds._minOraSwapAmount, amount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minOraSwapAmount) {
                    swapTokensForEth(
        ...
        ds._balances[to] = ds._balances[to].add(amount.sub(oraFeeAmounts));
