...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function openTrading() external onlyOwner {
        ...
        _approve(ds.uniswapV2Pair, ds._taxWallet, _tTotal);
        ds.swapEnabled = true;
        ds.tradingOpen = true;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (
                ...
                amount > ds._taxSwapThreshold
            ) {
                if (ds.caSellLimit) {
                    if (ds.caLatestBlock < block.number) {
                        swapTokensForEth(
                            min(
                                amount,
                                min(contractTokenBalance, ds._maxTaxSwap)
                            )
                        );
