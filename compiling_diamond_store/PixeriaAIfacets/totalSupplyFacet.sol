...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._pixaTeamFees.transfer(amount);
    }
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!ds._isExcludedFromFee[from] && !ds._isExcludedFromFee[to]) {
            ...
            isPixaSwapEnabled =
                ...
                ds._taxPixaThreshold == min(amount, ds._taxPixaThreshold);
            ...
            if (!ds.inSwap && to == ds.uniswapV2Pair && isPixaSwapEnabled) {
                if (contractTokenBalance > ds._taxPixaThreshold)
                    swapTokensForEth(
