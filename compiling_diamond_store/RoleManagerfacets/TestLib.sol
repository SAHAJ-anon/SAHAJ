...
bytes32 constant _name_ = bytes32(
    abi.encodePacked("Yearn V3 Vault Role Manager")
);
bytes32 constant DADDY = keccak256("Daddy");
bytes32 constant BRAIN = keccak256("Brain");
bytes32 constant KEEPER = keccak256("Keeper");
bytes32 constant SECURITY = keccak256("Security");
bytes32 constant REGISTRY = keccak256("Registry");
bytes32 constant ACCOUNTANT = keccak256("Accountant");
bytes32 constant DEBT_ALLOCATOR = keccak256("Debt Allocator");
bytes32 constant STRATEGY_MANAGER = keccak256("Strategy Manager");
...
library TestLib {
    ...
    struct TestStorage {
        address chad;
        address[] vaults;
        uint256 defaultProfitMaxUnlock;
        mapping(bytes32 => undefined) _positions;
        mapping(address => undefined) vaultConfig;
        mapping(address => mapping(string => mapping(uint256 => address))) _assetToVault;
    }
