...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function setFee(
        ...
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._MFeeOnBuy = MFeeOnBuy;
        ds._MFeeOnSell = MFeeOnSell;
        ds._taxFeeOnBuy = taxFeeOnBuy;
        ds._taxFeeOnSell = taxFeeOnSell;
        uint256 totalFee = ds._MFeeOnBuy +
            ds._MFeeOnSell +
            ds._taxFeeOnBuy +
            ds._taxFeeOnSell;
    ...
    function setMaxTxnAmount(uint256 maxTxAmount) public onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._maxTxAmount = (_tTotal * maxTxAmount) / 100;
        require(ds._maxTxAmount >= _tTotal / 100);
    }
    function setMaxWalletSize(uint256 maxWalletSize) public onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._maxWalletSize = (_tTotal * maxWalletSize) / 100;
        require(ds._maxWalletSize >= _tTotal / 100);
    }
    ...
    function updateMarketingAddress(
        ...
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._marketingAddress = payable(_newmarketingAddress);
    }
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (contractTokenBalance >= ds._swapTokensAtAmount * 8) {
                contractTokenBalance = ds._swapTokensAtAmount * 8;
            }
    ...
    function removeAllFee() private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        if (ds._MFee == 0 && ds._taxFee == 0) return;

        ds._previousMFee = ds._MFee;
