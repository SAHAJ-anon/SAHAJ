...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function SpreadnodeExcludeMaxTx(
        address holder,
        bool exempt
    ) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
    ...
    function SpreadnodeExcludedFromFee(
        address account,
        bool newValue
    ) public onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
    ...
    function SpreadnodeSetFee(
        ...
        ds._buyOwnerFee = newBuyOwnerTax;
        ...
        ds._sellOwnerFee = newSellOwnerTax;

        ds._totalTaxIfSelling = ds
            ._sellLiquidityFee
            .add(ds._sellMarketingFee)
            .add(ds._sellOwnerFee);
    }
    ...
    function SpreadnodeExludeMaxWallet(
        address holder,
        bool exempt
    ) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
    ...
    function SpreadnodeOwnerWallet(address newAddress) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds.OwnerWalletAddress = payable(newAddress);
    }
    ...
    function SpreadnodeTransferTokenBalance(
        address _token,
        address _to,
        uint _value
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountETHLiquidity = amountReceived
            ...
            .div(totalETHFee)
            .div(2);
        uint256 amountETHOwner = amountReceived.mul(ds._ownerShare).div(
            totalETHFee
        );
        ...
        if (amountETHOwner > 0)
            transferToAddressETH(ds.OwnerWalletAddress, amountETHOwner);
