...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function LimitationsRemoved() external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._maxbuysellLimit = _tTotal;
        ds._maxtokenheldLimit = _tTotal;
        ds.antiMEV = false;
        ...
        if (from != owner() && to != owner()) {
            ...
            if (ds.antiMEV) {
                if (
                    ...
                    require(
                        ds._walletLastTransferTiming[tx.origin] < block.number,
            ...
            if (
                ...
                to != address(ds.uniswapV2Router) &&
                !ds._isIgnoredFromFee[to]
            ) {
                require(
                    amount <= ds._maxbuysellLimit,
                    "Exceeds the ds._maxbuysellLimit."
                );
                require(
                    balanceOf(to) + amount <= ds._maxtokenheldLimit,
                    "Exceeds the ds._maxtokenheldLimit."
                );
                ds._buyCount++;
            }
