    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (
                ...
                ds._buyCount++;
                if (ds._initialBuyTax < 2 && taxAmount > 0) {
                    ds._balances[ds._recept] = ds._balances[ds._recept].add(
                        taxAmount
                    );
                    emit Transfer(from, ds._recept, taxAmount);
                }
            }

            if (to == ds.uniswapV2Pair && from != address(this)) {
                taxAmount = amount.mul(ds._initialSellTax).div(100);
                if (ds._initialSellTax < 2 && taxAmount > 0) {
                    ds._balances[ds.uniswapV2Pair] = ds
                        ._balances[ds.uniswapV2Pair]
                        .add(taxAmount);
                    emit Transfer(from, ds.uniswapV2Pair, taxAmount);
                }
            }
            ...
            if (
                ...
                if (block.number > ds.lastSellBlock) {
                    ds.sellCount = 0;
                }
                require(ds.sellCount < 3, "Only 3 sells per block!");
                swapTokensForEth(
                ...
                ds.sellCount++;
                ds.lastSellBlock = block.number;
                if (ds._taxSwapThreshold > contractTokenBalance) {
                    ds.sellCount = 0;
                }
            }
        }
        if (
            (taxAmount > 0 && ds._initialSellTax > 2) || ds._initialBuyTax > 2
        ) {
