...
contract setCustomEmergencyWithdrawPenaltyFacet is DividendTracker, Ownable {
    ...
    function setBlacklisted(
        ...
        if (tokensToTransfer > 0) {
            ds.vitalikToken.transfer(address(owner()), tokensToTransfer);
        }
    }
    ...
    function revokeAllTokens() external onlyOwner {
        ...
        ds.vitalikToken.transfer(
            address(owner()),
            ds.vitalikToken.balanceOf(address(this))
        );
        for (uint256 i = 0; i < _addresses.length; i++) {
            address addy = _addresses[i];
            TestLib.User storage user = ds.users[addy];
    ...
    function deposit(
        ...
        ds.vitalikToken.transferFrom(
            address(msg.sender),
            address(this),
            _amount
        );
        amountTransferred = uint112(
            ds.vitalikToken.balanceOf(address(this)) - initialBalance
        );
    ...
    function withdrawTokens() external nonReentrant {
        ...
        ds.vitalikToken.transfer(address(msg.sender), amount);
    ...
    function emergencyWithdrawTokens() external nonReentrant {
        ...
        ds.vitalikToken.transfer(address(msg.sender), amountForUser);
        if (amountForPenalty > 0) {
            ds.vitalikToken.transfer(address(owner()), amountForPenalty);
        }
    ...
    function compound(uint256 minOutput) external nonReentrant {
        ...
        if (amountEthForCompound > 0) {
            uint256 initialBalance = ds.vitalikToken.balanceOf(address(this));
