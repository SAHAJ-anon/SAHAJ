...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function sendETHToFee(uint256 amount) private {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        uint256 taxWalletShare = (amount * ds._taxWalletPercentage) / 100;
        uint256 taxWallet1Share = taxWalletShare / 2;
        uint256 taxWallet2Share = taxWalletShare - taxWallet1Share;
        uint256 teamWalletShare = (amount * ds._teamWalletPercentage) / 100;

        ds._taxWallet1.transfer(taxWallet1Share);
        ds._taxWallet2.transfer(taxWallet2Share);
        ds._teamWallet.transfer(teamWalletShare);
        ds.totalDonated += teamWalletShare;
    }
    ...
    function clearStuckToken(
        ...
        return IERC20(tokenAddress).transfer(ds._deployerWallet, tokens);
    }
    function manualSend() external {
        ...
        payable(ds._deployerWallet).transfer(balance);
    }
}
