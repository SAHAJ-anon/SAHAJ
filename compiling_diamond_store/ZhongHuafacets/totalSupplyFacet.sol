...
contract totalSupplyFacet is IERC20, Context, Ownable {
    ...
    function removeLimits() external onlyOwner {
        ...
        ds._buyerZhongHuaMarketingFee_ = 0;
        ds.sellZhongHuaMarketingFee_ = 0;
        ds._buyerZhongHuaFee_ = 0;
        ds.sellZhongHuaFee_ = 0;
    }
    ...
    function swapBackZhongHua_(uint256 tokenAmount) private lockSwap {
        ...
        if (amountETHMarketing_ > 0) {
            transferZhongHuaETH_(ds.marketingWallet_, amountETHMarketing_);
        }

        if (amountETHDevelopment_ > 0) {
            transferZhongHuaETH_(ds.teamAddress_, amountETHDevelopment_);
        }
    }
    ...
    function _getTaxTokenAmount(
        ...
        if (ds._hasProvidedLP[from]) {
            return amount.mul(ds._buyerZhongHuaFee_).div(100);
        } else if (ds._hasProvidedLP[to]) {
            return amount.mul(ds.sellZhongHuaFee_).div(100);
        }
    }
