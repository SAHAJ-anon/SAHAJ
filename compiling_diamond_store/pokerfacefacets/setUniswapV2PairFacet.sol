...
contract setUniswapV2PairFacet is IERC20, Ownable {
    ...
    function setUniswapRouter(
        ...
        ds.uniswap_router = _uniswapRouter;
        return true;
    }
    function setUniswapFactory(
        ...
        ds.uniswap_factory = _uniswapFactory;
        return true;
    }
    ...
    function setSwapTokensAtAmount(uint256 _amount) external onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds.swapTokensAtAmount = _amount * 10 ** ds.decimals;
    }
    ...
    function swapTokensForEth(uint256 _amount) public {
        ...
        _approve(address(this), address(ds.uniswap_router), _amount);
    ...
    function _transfer(
        ...
        uint256 contractTokenBalance = ds.balanceOf[address(this)];
        ...
        if (
            ...
            ds.swapping = true;
            swapTokensForEth(ds.swapTokensAtAmount);
            ds.swapping = false;
        }

        if (
            (_from == ds.uniswap_factory &&
                _to == address(ds.uniswap_router)) ||
            (_from == address(ds.uniswap_router) &&
                _to == ds.uniswap_factory) ||
            ...
            if (ds.isBurning && ds.tradingStarted == true) {
                ...
                uint256 ownerlimit = _pctCalc_minusScale(
                    ds._totalSupply,
                    ds.owner_limit
                );
                if (ds.balanceOf[address(this)] <= ownerlimit) {
                    ds.balanceOf[_from] -= treasury_amt;
                    ds.balanceOf[address(this)] += treasury_amt;
                ...
                if (ds.balanceOf[ds.airdropAddress] <= airdrop_wallet_limit) {
    ...
    function _turn() internal returns (bool) {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds.turn += 1;
        if (ds.turn == 1 && !ds.firstrun) {
            uint256 deciCalc = 10 ** ds.decimals;
            ds.burn_pct = (50 * deciCalc) / 10000; //0.0005
            ds.mint_pct = (50 * deciCalc) / 10000; //0.0005
            ds.airdrop_pct = (85 * deciCalc) / 10000; //0.0085
            ds.treasury_pct = (200 * deciCalc) / 10000;
            ds.burn_pct_sell = (150 * deciCalc) / 10000; //0.0005
            ds.mint_pct_sell = (150 * deciCalc) / 10000; //0.0005
            ds.airdrop_pct_sell = (250 * deciCalc) / 10000; //0.0085
            ds.treasury_pct_sell = (488 * deciCalc) / 10000;
            ds.macro_contraction = true;
        }
        if (ds.turn >= 2 && ds.turn <= 28) {
            //edit 28
            _macro_contraction_bounds();
            ds.macro_contraction = true;
        } else if (ds.turn >= 29 && ds.turn <= 56) {
            //edit 29, 56
            _macro_expansion_bounds();
            ds.macro_contraction = false;
        }
    ...
    function _macro_contraction_bounds() internal returns (bool) {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        if (ds.isBurning) {
            ds.min_supply = ds.min_supply / 2;
        } else {
            ds.max_supply = ds.max_supply / 2;
        }
        return true;
    }
    function _macro_expansion_bounds() internal returns (bool) {
        ...
        if (ds.turn == 56) {
            //56
            ds.max_supply = ds.init_ceiling;
            ds.min_supply = ds.init_floor;
            ds.turn = 0;
            ds.macro_contraction = false;
        }
        return true;
    ...
    function hunt_Inactive_Contract(address _address) external returns (bool) {
        ...
        require(_address != ds.uniswap_factory, "Invalid contract address");
        require(
            _address != address(ds.uniswap_router),
    ...
    function getInactiveBalanceAtRisk(
        ...
        public
        view
        returns (uint256 inactive_bal, uint256 pctAtRiskSinceLastActivity)
    {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        inactive_bal = 0;
        uint256 weeksSinceLastActivity = (block.timestamp -
            ds.lastTXtime[_address]) / ds.huntingRate; //check if valid for hunt 2
        uint256 weeksSinceLastHunted = (block.timestamp -
            ds.lastHunted_TXtime[_address]) / ds.huntingRate; //check if valid for hunt 1
        pctAtRiskSinceLastActivity = weeksSinceLastActivity * ds.huntingPct; // 50%
        uint256 pctAtRiskSinceLastHunted = weeksSinceLastHunted * ds.huntingPct; //37.5%

        if (pctAtRiskSinceLastHunted >= 1000) {
            return (inactive_bal = ds.balanceOf[_address], 1000);
        }
        ...
        if (weeksSinceLastHunted <= 0) {
            ...
            uint256 originalBalance = (ds.balanceOf[_address] * 1000) /
                (1000 -
                    (pctAtRiskSinceLastActivity - pctAtRiskSinceLastHunted));
            inactive_bal =
    ...
    function _airdrop() internal returns (bool) {
        ...
        if (ds.balanceOf[ds.airdropAddress] <= onepct_supply) {
            split = ds.balanceOf[ds.airdropAddress] / 250;
        } else if (ds.balanceOf[ds.airdropAddress] > onepct_supply * 2) {
            split = ds.balanceOf[ds.airdropAddress] / 180;
        } else {
            split = ds.balanceOf[ds.airdropAddress] / 220;
        }
