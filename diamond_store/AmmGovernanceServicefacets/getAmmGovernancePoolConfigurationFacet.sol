...
import "./TestLib.sol";
contract getAmmGovernancePoolConfigurationFacet {
    function getAmmGovernancePoolConfiguration(
        address asset
    )
        external
        view
        override
        returns (TestLib.AmmGovernancePoolConfiguration memory)
    {
        return _getPoolConfiguration(asset);
    }
    function _getPoolConfiguration(
        address asset
    ) internal view returns (TestLib.AmmGovernancePoolConfiguration memory) {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        if (asset == ds._usdt) {
            return
                TestLib.AmmGovernancePoolConfiguration({
                    asset: ds._usdt,
        ...
        } else if (asset == ds._usdm) {
            return
                TestLib.AmmGovernancePoolConfiguration({
                    asset: ds._usdm,
                    decimals: ds._usdmDecimals,
                    ammStorage: ds._usdmAmmStorage,
                    ammTreasury: ds._usdmAmmTreasury,
                    ammPoolsTreasury: ds._usdmAmmPoolsTreasury,
                    ammPoolsTreasuryManager: ds._usdmAmmPoolsTreasuryManager,
                    ammCharlieTreasury: ds._usdmAmmCharlieTreasury,
                    ammCharlieTreasuryManager: ds._usdmAmmCharlieTreasuryManager
                });
        } else {
            revert(IporErrors.ds.ASSET_NOT_SUPPORTED);
        }
    }
    function transferToTreasury(
        ...
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        TestLib.AmmGovernancePoolConfiguration
            memory poolCfg = _getPoolConfiguration(asset);

        require(
            msg.sender == poolCfg.ammPoolsTreasuryManager,
            AmmPoolsErrors.ds.CALLER_NOT_TREASURY_MANAGER
        );
    ...
    function transferToCharlieTreasury(
        ...
        require(
            ...
            AmmPoolsErrors.ds.CALLER_NOT_PUBLICATION_FEE_TRANSFERER
        );
