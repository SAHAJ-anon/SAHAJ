...
import "./TestLib.sol";
contract isSwapLiquidatorFacet {
    function isSwapLiquidator(
        address asset,
        address account
    ...
    function getAmmSwapsLiquidatorsStorage()
        internal
        pure
        returns (TestLib.AmmSwapsLiquidatorsStorage storage store)
    {
        uint256 slot = _getStorageSlot(TestLib.StorageId.AmmSwapsLiquidators);
        assembly {
            store.slot := slot
        }
    }
    function _getStorageSlot(
        TestLib.StorageId storageId
    ) private pure returns (uint256 slot) {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        return uint256(storageId) + ds.STORAGE_SLOT_BASE;
    }
    function getOwner()
        internal
        pure
        returns (TestLib.OwnerStorage storage owner)
    {
        uint256 slot = _getStorageSlot(TestLib.StorageId.Owner);
        assembly {
            owner.slot := slot
        }
    }
    function getAppointedOwner()
        internal
        pure
        returns (TestLib.AppointedOwnerStorage storage appointedOwner)
    {
        uint256 slot = _getStorageSlot(TestLib.StorageId.AppointedOwner);
        assembly {
            appointedOwner.slot := slot
        }
    }
    function getReentrancyStatus()
        internal
        pure
        returns (TestLib.ReentrancyStatusStorage storage reentrancyStatus)
    {
        uint256 slot = _getStorageSlot(TestLib.StorageId.ReentrancyStatus);
        assembly {
            reentrancyStatus.slot := slot
        }
    }
    function getRouterFunctionPaused()
        internal
        pure
        returns (TestLib.RouterFunctionPausedStorage storage paused)
    {
        uint256 slot = _getStorageSlot(TestLib.StorageId.RouterFunctionPaused);
        assembly {
            paused.slot := slot
        }
    }
    function getPauseGuardianStorage()
        ...
        returns (mapping(address => bool) storage store)
    {
        uint256 slot = _getStorageSlot(TestLib.StorageId.PauseGuardian);
        assembly {
            store.slot := slot
        }
    }
    function getAmmPoolsAppointedToRebalanceStorage()
        internal
        pure
        returns (TestLib.AmmPoolsAppointedToRebalanceStorage storage store)
    {
        uint256 slot = _getStorageSlot(
            TestLib.StorageId.AmmPoolsAppointedToRebalance
        );
        assembly {
            store.slot := slot
        }
    ...
    function getAmmPoolsParamsStorage()
        internal
        pure
        returns (TestLib.AmmPoolsParamsStorage storage store)
    {
        uint256 slot = _getStorageSlot(TestLib.StorageId.AmmPoolsParams);
        assembly {
            store.slot := slot
        }
    }
    ...
    function getAmmPoolsParams(
        ...
        view
        override
        returns (TestLib.AmmPoolsParamsConfiguration memory cfg)
    {
        StorageLib.TestLib.AmmPoolsParamsValue
            memory ammPoolsParamsCfg = AmmConfigurationManager
                .getAmmPoolsParams(asset);
        cfg = TestLib.AmmPoolsParamsConfiguration({
