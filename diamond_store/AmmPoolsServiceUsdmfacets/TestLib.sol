...
interface IAmmPoolsServiceUsdm {
    ...
    function redeemFromAmmPoolUsdm(
        address beneficiary,
...
interface IAssetManagement {
    ...
    function deposit(
        uint256 amount
    ) external returns (uint256 vaultBalance, uint256 depositedAmount);
    ...
    function withdraw(
        uint256 amount
    ) external returns (uint256 withdrawnAmount, uint256 vaultBalance);
...
library StorageLib {
    ...
    function getReentrancyStatus()
        internal
        pure
        returns (ReentrancyStatusStorage storage reentrancyStatus)
    {
    ...
    function getRouterFunctionPaused()
        internal
        pure
        returns (RouterFunctionPausedStorage storage paused)
    {
    ...
    function getPauseGuardianStorage()
        internal
        pure
        returns (mapping(address => bool) storage store)
    {
    ...
    function getAmmSwapsLiquidatorsStorage()
        internal
        pure
        returns (AmmSwapsLiquidatorsStorage storage store)
    {
    ...
    function getAmmPoolsParamsStorage()
        internal
        pure
        returns (AmmPoolsParamsStorage storage store)
    {
...
library AmmErrors {
    ...
    string public constant SWAP_NOTIONAL_HIGHER_THAN_TOTAL_NOTIONAL =
        "IPOR_314";
    ...
    string public constant MAX_LENGTH_LIQUIDATED_SWAPS_PER_LEG_EXCEEDED =
        "IPOR_315";
    ...
    string public constant CALC_TIMESTAMP_LOWER_THAN_SOAP_REBALANCE_TIMESTAMP =
        "IPOR_317";

    /// @notice Calculation timestamp is lower than  Swap's open timestamp.
    string public constant CALC_TIMESTAMP_LOWER_THAN_SWAP_OPEN_TIMESTAMP =
        "IPOR_318";

    /// @notice Closing timestamp is lower than Swap's open timestamp.
    string public constant CLOSING_TIMESTAMP_LOWER_THAN_SWAP_OPEN_TIMESTAMP =
        "IPOR_319";

    /// @notice Swap cannot be closed because sender is not a buyer nor liquidator.
    string
        public constant CANNOT_CLOSE_SWAP_SENDER_IS_NOT_BUYER_NOR_LIQUIDATOR =
        "IPOR_320";

    /// @notice Interest from Strategy is below zero.
    string public constant INTEREST_FROM_STRATEGY_EXCEEDED_THRESHOLD =
        "IPOR_321";
    ...
    string public constant CANNOT_CLOSE_SWAP_CLOSING_IS_TOO_EARLY_FOR_BUYER =
        "IPOR_328";
    ...
    string public constant STORAGE_ID_IS_NOT_TIME_WEIGHTED_NOTIONAL =
        "IPOR_332";
    ...
    string public constant AVERAGE_INTEREST_RATE_WHEN_OPEN_SWAP_CANNOT_BE_ZERO =
        "IPOR_336";

    /// @notice Average interest rate cannot be zero when close swap
    string
        public constant AVERAGE_INTEREST_RATE_WHEN_CLOSE_SWAP_CANNOT_BE_ZERO =
        "IPOR_337";
    ...
    string public constant COLLATERAL_IS_NOT_SUFFICIENT_TO_COVER_UNWIND_SWAP =
        "IPOR_339";
    ...
    string public constant CANNOT_CLOSE_SWAP_WITH_UNWIND_ACTION_IS_TOO_EARLY =
        "IPOR_341";
}
...
library AmmPoolsErrors {
    ...
    string public constant CFG_INVALID_REDEEM_LP_MAX_COLLATERAL_RATIO =
        "IPOR_412";
}
...
interface IIporOracle {
    ...
    function updateIndexes(
        UpdateIndexParams[] calldata indexesToUpdate
    ) external;
...
interface IAmmCloseSwapLens {
    ...
    function getClosingSwapDetails(
        ...
        external
        view
        returns (AmmTypes.ClosingSwapDetails memory closingSwapDetails);
}
...
library AmmConfigurationManager {
    ...
    function addSwapLiquidator(address asset, address account) internal {
        ...
        mapping(address => mapping(address => bool))
            storage swapLiquidators = StorageLib
    ...
    function addAppointedToRebalanceInAmm(
        ...
        mapping(address => mapping(address => bool))
            storage appointedToRebalance = StorageLib
    ...
    function setAmmPoolsParams(
        ...
        require(
            newAmmTreasuryAndAssetManagementRatio < 1e4,
            AmmPoolsErrors.AMM_TREASURY_ASSET_MANAGEMENT_RATIO
        );

        StorageLib.getAmmPoolsParamsStorage().value[asset] = StorageLib
            .AmmPoolsParamsValue({
    ...
    function getAmmPoolsParams(
        address asset
    ) internal view returns (StorageLib.AmmPoolsParamsValue memory) {
...
interface IAmmStorage {
    ...
    function getConfiguration()
        external
        view
        returns (address ammTreasury, address router);
    ...
    function getExtendedBalance()
        external
        view
        returns (AmmStorageTypes.ExtendedBalancesMemory memory);
    ...
    function getSwap(
        AmmTypes.SwapDirection direction,
        uint256 swapId
    ) external view returns (AmmTypes.Swap memory);
    ...
    function updateStorageWhenTransferToTreasuryInternal(
        uint256 transferredAmount
    ) external;
}
...
library AmmLib {
        ...
        returns (int256 soapPayFixed, int256 soapReceiveFixed, int256 soap)
    {
        ...
        uint256 ibtPrice = IIporOracle(model.iporOracle)
            .calculateAccruedIbtPrice(model.asset, timestamp);
        soapPayFixed = indicatorsPayFixed.calculateSoapPayFixed(
            timestamp,
            ibtPrice
        );
        soapReceiveFixed = indicatorsReceiveFixed.calculateSoapReceiveFixed(
            timestamp,
            ibtPrice
        );
    ...
    function getAccruedBalance(
        ...
        require(
            model.ammTreasury != address(0),
            string.concat(IporErrors.WRONG_ADDRESS, " ammTreasury")
        );
        IporTypes.AmmBalancesMemory memory accruedBalance = IAmmStorage(
            model.ammStorage
        ).getBalance();

        uint256 actualVaultBalance = IAssetManagement(model.assetManagement)
            .totalBalance();
...
library TestLib {
    ...
    struct TestStorage {
        ...
        string IP_TOKEN_MINT_AMOUNT_TOO_LOW;
        string IP_TOKEN_BURN_AMOUNT_TOO_LOW;
        string REDEEM_LP_COLLATERAL_RATIO_EXCEEDED;
        string CANNOT_REDEEM_IP_TOKEN_TOO_LOW;
        string CALLER_NOT_TREASURY_MANAGER;
        string CANNOT_REDEEM_ASSET_AMOUNT_TOO_LOW;
        string CALLER_NOT_PUBLICATION_FEE_TRANSFERER;
        string ASSET_MANAGEMENT_BALANCE_IS_EMPTY;
        string AMM_TREASURY_ASSET_MANAGEMENT_RATIO;
        string INSUFFICIENT_ERC20_BALANCE;
        string CALLER_NOT_APPOINTED_TO_REBALANCE;
        string CFG_INVALID_REDEEM_FEE_RATE;
        string CFG_INVALID_REDEEM_LP_MAX_COLLATERAL_RATIO;
        string WRONG_ADDRESS;
        string WRONG_DECIMALS;
        string ADDRESSES_MISMATCH;
        string SENDER_ASSET_BALANCE_TOO_LOW;
        string VALUE_NOT_GREATER_THAN_ZERO;
        string INPUT_ARRAYS_LENGTH_MISMATCH;
        string NOT_ENOUGH_AMOUNT_TO_TRANSFER;
        string SENDER_NOT_APPOINTED_OWNER;
        string CALLER_NOT_IPOR_PROTOCOL_ROUTER;
        string CHUNK_SIZE_EQUAL_ZERO;
        string CHUNK_SIZE_TOO_BIG;
        string CALLER_NOT_PAUSE_GUARDIAN;
        string ROUTER_INVALID_SIGNATURE;
        string CALLER_NOT_AMM_TREASURY;
        string CALLER_NOT_OWNER;
        string METHOD_PAUSED;
        string REENTRANCY;
        string ASSET_NOT_SUPPORTED;
        string ROUTER_RETURN_BACK_ETH_FAILED;
        string RISK_INDICATORS_EXPIRED;
        string RISK_INDICATORS_SIGNATURE_INVALID;
        string INPUT_ASSET_NOT_SUPPORTED;
        string UNSUPPORTED_MODULE_ASSET_MANAGEMENT;
        uint256 RAY;
        ...
        address usdm;
        address ipUsdm;
        address ammTreasuryUsdm;
        address ammStorageUsdm;
        address iporOracle;
        address iporProtocolRouter;
        uint256 redeemFeeRateUsdm;
    }
