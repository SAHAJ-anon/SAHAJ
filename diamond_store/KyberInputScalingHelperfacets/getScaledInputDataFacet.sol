...
import "./TestLib.sol";
contract getScaledInputDataFacet {
    function getScaledInputData(
        ...
        if (selector == IMetaAggregationRouterV2.swap.selector) {
            IMetaAggregationRouterV2.TestLib.SwapExecutionParams
                memory params = abi.decode(
                    dataToDecode,
                    (IMetaAggregationRouterV2.TestLib.SwapExecutionParams)
                );
            ...
            (params.desc, params.targetData) = _getScaledInputDataV2(
                ...
                params.targetData,
                newAmount,
                _flagsChecked(params.desc.flags, ds._SIMPLE_SWAP)
            );
            ...
            (
                address callTarget,
                IMetaAggregationRouterV2.TestLib.SwapDescriptionV2 memory desc,
                ...
                bytes memory clientData
                    ...
                    (
                        address,
                        IMetaAggregationRouterV2.TestLib.SwapDescriptionV2,
                        bytes,
                        bytes
                    )
                );

            (desc, targetData) = _getScaledInputDataV2(
                desc,
                targetData,
                newAmount,
                true
    ...
    function _getScaledInputDataV2(
        ...
        pure
        returns (
            IMetaAggregationRouterV2.TestLib.SwapDescriptionV2 memory,
            bytes memory
        )
    {
    ...
    function _scaledSwapDescriptionV2(
        ...
        internal
        pure
        returns (IMetaAggregationRouterV2.TestLib.SwapDescriptionV2 memory)
    {
    ...
    function _scaledSimpleSwapData(
        ...
        TestLib.SimpleSwapData memory swapData = abi.decode(
            data,
            (TestLib.SimpleSwapData)
        );
    ...
    function _scaledPositiveSlippageFeeData(
        ...
        if (data.length > 32) {
            TestLib.PositiveSlippageFeeData memory psData = abi.decode(
                data,
                (TestLib.PositiveSlippageFeeData)
            );
            uint256 left = uint256(psData.expectedReturnAmount >> 128);
            uint256 right = (uint256(uint128(psData.expectedReturnAmount)) *
                newAmount) / oldAmount;
            ...
            uint256 right = (uint256(uint128(expectedReturnAmount)) *
                newAmount) / oldAmount;
            require(right <= type(uint128).max, "Exceeded type range");
    ...
    function _scaledExecutorCallBytesData(
        ...
        TestLib.SwapExecutorDescription memory executorDesc = abi.decode(
            data,
            (TestLib.SwapExecutorDescription)
        );
        ...
        for (uint256 i = 0; i < nSequences; ) {
            TestLib.Swap memory swap = executorDesc.swapSequences[i][0];
            ...
            if (functionSelector == IExecutorHelper.executeUniswap.selector) {
                ...
                functionSelector == IExecutorHelper.executeStableSwap.selector
            ) {
                ...
                functionSelector ==
                IExecutorHelper.executeUniV3KSElastic.selector
            ) {
                swap.data = newUniV3ProMM(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeRfq.selector
            ) {
                revert("InputScalingHelper: Can not scale RFQ swap");
            } else if (
                functionSelector == IExecutorHelper.executeBalV2.selector
            ) {
                swap.data = newBalancerV2(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeWrappedstETH.selector
            ) {
                swap.data = newWrappedstETHSwap(
                    swap.data,
                    oldAmount,
                    newAmount
                );
                ...
                functionSelector == IExecutorHelper.executeDODO.selector
            ) {
                ...
                functionSelector == IExecutorHelper.executeGMX.selector
            ) {
                swap.data = newGMX(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeSynthetix.selector
            ) {
                swap.data = newSynthetix(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeHashflow.selector
            ) {
                revert("InputScalingHelper: Can not scale Hasflow swap");
            } else if (
                functionSelector == IExecutorHelper.executeCamelot.selector
            ) {
                swap.data = newCamelot(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector ==
                IExecutorHelper.executeKyberLimitOrder.selector
            ) {
                revert("InputScalingHelper: Can not scale KyberLO swap");
            } else if (
                functionSelector == IExecutorHelper.executePSM.selector
            ) {
                ...
                functionSelector == IExecutorHelper.executePlatypus.selector
            ) {
                swap.data = newPlatypus(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeMaverick.selector
            ) {
                swap.data = newMaverick(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeSyncSwap.selector
            ) {
                swap.data = newSyncSwap(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeAlgebraV1.selector
            ) {
                swap.data = newAlgebraV1(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector ==
                IExecutorHelper.executeBalancerBatch.selector
            ) {
                swap.data = newBalancerBatch(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeWombat.selector
            ) {
                swap.data = newMantis(swap.data, oldAmount, newAmount); // @dev struct TestLib.Mantis is used for both Wombat and TestLib.Mantis because of same fields
            } else if (
                functionSelector == IExecutorHelper.executeMantis.selector
            ) {
                swap.data = newMantis(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeIziSwap.selector
            ) {
                swap.data = newIziSwap(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeWooFiV2.selector
            ) {
                swap.data = newMantis(swap.data, oldAmount, newAmount); // @dev using TestLib.Mantis struct because WooFiV2 and TestLib.Mantis have same fields
            } else if (
                functionSelector == IExecutorHelper.executeTraderJoeV2.selector
            ) {
                swap.data = newTraderJoeV2(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector ==
                IExecutorHelper.executePancakeStableSwap.selector
            ) {
                swap.data = newCurveSwap(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeLevelFiV2.selector
            ) {
                swap.data = newLevelFiV2(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeGMXGLP.selector
            ) {
                swap.data = newGMXGLP(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeVooi.selector
            ) {
                swap.data = newVooi(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeVelocoreV2.selector
            ) {
                swap.data = newVelocoreV2(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeMaticMigrate.selector
            ) {
                swap.data = newMaticMigrate(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeSmardex.selector
            ) {
                swap.data = newMantis(swap.data, oldAmount, newAmount); // @dev using TestLib.Mantis struct because Smardex and TestLib.Mantis have same fields
            } else if (
                functionSelector == IExecutorHelper.executeSolidlyV2.selector
            ) {
                swap.data = newMantis(swap.data, oldAmount, newAmount); // @dev using TestLib.Mantis struct because Solidly V2 and TestLib.Mantis have same fields
            } else if (
                functionSelector == IExecutorHelper.executeKokonut.selector
            ) {
                swap.data = newKokonut(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeBalancerV1.selector
            ) {
                swap.data = newBalancerV1(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeSwaapV2.selector
            ) {
                revert(
                    "InputScalingHelper: Can not scale TestLib.SwaapV2 swap"
                );
            } else if (
                functionSelector ==
                IExecutorHelper.executeNomiswapStable.selector
            ) {
                swap.data = newMantis(swap.data, oldAmount, newAmount); // @dev using TestLib.Mantis struct because NomiswapV2 and TestLib.Mantis have same fields
            } else if (
                functionSelector ==
                IExecutorHelper.executeArbswapStable.selector
            ) {
                swap.data = newArbswapStable(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeBancorV2.selector
            ) {
                swap.data = newBancorV2(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeBancorV3.selector
            ) {
                swap.data = newMantis(swap.data, oldAmount, newAmount); // @dev using TestLib.Mantis struct because Bancor V3 and TestLib.Mantis have same fields
            } else if (
                functionSelector == IExecutorHelper.executeAmbient.selector
            ) {
                swap.data = newAmbient(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeNative.selector
            ) {
                revert("InputScalingHelper: Can not scale Native swap");
            } else if (
                functionSelector == IExecutorHelper.executeLighterV2.selector
            ) {
                swap.data = newLighterV2(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeBebop.selector
            ) {
                revert("InputScalingHelper: Can not scale Bebop swap");
            } else if (
                functionSelector == IExecutorHelper.executeUniV1.selector
            ) {
                ...
                functionSelector == IExecutorHelper.executeEtherFiWeETH.selector
            ) {
                swap.data = newEtherFiWeETH(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeKelp.selector
            ) {
                swap.data = newKelp(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeEthenaSusde.selector
            ) {
                swap.data = newEthenaSusde(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeRocketPool.selector
            ) {
                swap.data = newRocketPool(swap.data, oldAmount, newAmount);
            } else if (
                functionSelector == IExecutorHelper.executeMakersDAI.selector
            ) {
    ...
    function newUniSwap(
        ...
        IExecutorHelper.TestLib.UniSwap memory uniSwap = abi.decode(
            data,
            (IExecutorHelper.TestLib.UniSwap)
        );
    ...
    function newStableSwap(
        ...
        IExecutorHelper.TestLib.StableSwap memory stableSwap = abi.decode(
            data,
            (IExecutorHelper.TestLib.StableSwap)
        );
    ...
    function newCurveSwap(
        ...
        IExecutorHelper.TestLib.CurveSwap memory curveSwap = abi.decode(
            data,
            (IExecutorHelper.TestLib.CurveSwap)
        );
    ...
    function newKyberDMM(
        ...
        IExecutorHelper.TestLib.UniSwap memory kyberDMMSwap = abi.decode(
            data,
            (IExecutorHelper.TestLib.UniSwap)
        );
        kyberDMMSwap.collectAmount =
            (kyberDMMSwap.collectAmount * newAmount) /
            oldAmount;
    ...
    function newUniV3ProMM(
        ...
        IExecutorHelper.TestLib.UniswapV3KSElastic memory uniSwapV3ProMM = abi
            .decode(data, (IExecutorHelper.TestLib.UniswapV3KSElastic));
        uniSwapV3ProMM.swapAmount =
            (uniSwapV3ProMM.swapAmount * newAmount) /
            oldAmount;
    ...
    function newBalancerV2(
        ...
        IExecutorHelper.TestLib.BalancerV2 memory balancerV2 = abi.decode(
            data,
            (IExecutorHelper.TestLib.BalancerV2)
        );
    ...
    function newWrappedstETHSwap(
        ...
        IExecutorHelper.TestLib.WSTETH memory wstEthData = abi.decode(
            data,
            (IExecutorHelper.TestLib.WSTETH)
        );
    ...
    function newDODO(
        ...
        IExecutorHelper.TestLib.DODO memory dodo = abi.decode(
            data,
            (IExecutorHelper.TestLib.DODO)
        );
    ...
    function newVelodrome(
        ...
        IExecutorHelper.TestLib.UniSwap memory velodrome = abi.decode(
            data,
            (IExecutorHelper.TestLib.UniSwap)
        );
        velodrome.collectAmount =
            (velodrome.collectAmount * newAmount) /
            oldAmount;
    ...
    function newGMX(
        ...
        IExecutorHelper.TestLib.GMX memory gmx = abi.decode(
            data,
            (IExecutorHelper.TestLib.GMX)
        );
    ...
    function newSynthetix(
        ...
        IExecutorHelper.TestLib.Synthetix memory synthetix = abi.decode(
            data,
            (IExecutorHelper.TestLib.Synthetix)
        );
        synthetix.sourceAmount =
            (synthetix.sourceAmount * newAmount) /
            oldAmount;
    ...
    function newCamelot(
        ...
        IExecutorHelper.TestLib.UniSwap memory camelot = abi.decode(
            data,
            (IExecutorHelper.TestLib.UniSwap)
        );
    ...
    function newPSM(
        ...
        IExecutorHelper.TestLib.PSM memory psm = abi.decode(
            data,
            (IExecutorHelper.TestLib.PSM)
        );
    ...
    function newFrax(
        ...
        IExecutorHelper.TestLib.UniSwap memory frax = abi.decode(
            data,
            (IExecutorHelper.TestLib.UniSwap)
        );
    ...
    function newPlatypus(
        ...
        IExecutorHelper.TestLib.Platypus memory platypus = abi.decode(
            data,
            (IExecutorHelper.TestLib.Platypus)
        );
        platypus.collectAmount =
            (platypus.collectAmount * newAmount) /
            oldAmount;
    ...
    function newMaverick(
        ...
        IExecutorHelper.TestLib.Maverick memory maverick = abi.decode(
            data,
            (IExecutorHelper.TestLib.Maverick)
        );
    ...
    function newSyncSwap(
        ...
        IExecutorHelper.TestLib.SyncSwap memory syncSwap = abi.decode(
            data,
            (IExecutorHelper.TestLib.SyncSwap)
        );
        syncSwap.collectAmount =
            (syncSwap.collectAmount * newAmount) /
            oldAmount;
    ...
    function newAlgebraV1(
        ...
        IExecutorHelper.TestLib.AlgebraV1 memory algebraV1Swap = abi.decode(
            data,
            (IExecutorHelper.TestLib.AlgebraV1)
        );
        algebraV1Swap.swapAmount =
            (algebraV1Swap.swapAmount * newAmount) /
            oldAmount;
    ...
    function newBalancerBatch(
        ...
        IExecutorHelper.TestLib.BalancerBatch memory balancerBatch = abi.decode(
            data,
            (IExecutorHelper.TestLib.BalancerBatch)
        );
        balancerBatch.amountIn =
            (balancerBatch.amountIn * newAmount) /
            oldAmount;
    ...
    function newMantis(
        ...
        IExecutorHelper.TestLib.Mantis memory mantis = abi.decode(
            data,
            (IExecutorHelper.TestLib.Mantis)
        );
    ...
    function newIziSwap(
        ...
        IExecutorHelper.TestLib.IziSwap memory iZi = abi.decode(
            data,
            (IExecutorHelper.TestLib.IziSwap)
        );
    ...
    function newTraderJoeV2(
        ...
        IExecutorHelper.TestLib.TraderJoeV2 memory traderJoe = abi.decode(
            data,
            (IExecutorHelper.TestLib.TraderJoeV2)
        );
        ...
        traderJoe.collectAmount =
            ...
            ((uint256((traderJoe.collectAmount << 1) >> 1) * newAmount) /
                oldAmount);
    ...
    function newLevelFiV2(
        ...
        IExecutorHelper.TestLib.LevelFiV2 memory levelFiV2 = abi.decode(
            data,
            (IExecutorHelper.TestLib.LevelFiV2)
        );
    ...
    function newGMXGLP(
        ...
        IExecutorHelper.TestLib.GMXGLP memory swapData = abi.decode(
            data,
            (IExecutorHelper.TestLib.GMXGLP)
        );
    ...
    function newVooi(
        ...
        IExecutorHelper.TestLib.Vooi memory vooi = abi.decode(
            data,
            (IExecutorHelper.TestLib.Vooi)
        );
    ...
    function newVelocoreV2(
        ...
        IExecutorHelper.TestLib.VelocoreV2 memory velocorev2 = abi.decode(
            data,
            (IExecutorHelper.TestLib.VelocoreV2)
        );
    ...
    function newMaticMigrate(
        ...
        IExecutorHelper.TestLib.MaticMigrate memory maticMigrate = abi.decode(
            data,
            (IExecutorHelper.TestLib.MaticMigrate)
        );
    ...
    function newKokonut(
        ...
        IExecutorHelper.TestLib.Kokonut memory kokonut = abi.decode(
            data,
            (IExecutorHelper.TestLib.Kokonut)
        );
    ...
    function newBalancerV1(
        ...
        IExecutorHelper.TestLib.BalancerV1 memory balancerV1 = abi.decode(
            data,
            (IExecutorHelper.TestLib.BalancerV1)
        );
    ...
    function newArbswapStable(
        ...
        IExecutorHelper.TestLib.ArbswapStable memory arbswapStable = abi.decode(
            data,
            (IExecutorHelper.TestLib.ArbswapStable)
        );
    ...
    function newBancorV2(
        ...
        IExecutorHelper.TestLib.BancorV2 memory bancorV2 = abi.decode(
            data,
            (IExecutorHelper.TestLib.BancorV2)
        );
    ...
    function newAmbient(
        ...
        IExecutorHelper.TestLib.Ambient memory ambient = abi.decode(
            data,
            (IExecutorHelper.TestLib.Ambient)
        );
    ...
    function newLighterV2(
        ...
        IExecutorHelper.TestLib.LighterV2 memory structData = abi.decode(
            data,
            (IExecutorHelper.TestLib.LighterV2)
        );
        structData.amount = uint128(
            (uint256(structData.amount) * newAmount) / oldAmount
        );
    ...
    function newUniV1(
        ...
        IExecutorHelper.TestLib.UniV1 memory structData = abi.decode(
            data,
            (IExecutorHelper.TestLib.UniV1)
        );
        structData.amount = uint128(
            (uint256(structData.amount) * newAmount) / oldAmount
        );
    ...
    function newEtherFiWeETH(
        ...
        IExecutorHelper.TestLib.EtherFiWeETH memory structData = abi.decode(
            data,
            (IExecutorHelper.TestLib.EtherFiWeETH)
        );
        structData.amount = uint128(
            (uint256(structData.amount) * newAmount) / oldAmount
        );
    ...
    function newKelp(
        ...
        IExecutorHelper.TestLib.Kelp memory structData = abi.decode(
            data,
            (IExecutorHelper.TestLib.Kelp)
        );
        structData.amount = uint128(
            (uint256(structData.amount) * newAmount) / oldAmount
        );
    ...
    function newEthenaSusde(
        ...
        IExecutorHelper.TestLib.EthenaSusde memory structData = abi.decode(
            data,
            (IExecutorHelper.TestLib.EthenaSusde)
        );
        structData.amount = uint128(
            (uint256(structData.amount) * newAmount) / oldAmount
        );
    ...
    function newRocketPool(
        ...
        IExecutorHelper.TestLib.RocketPool memory structData = abi.decode(
            data,
            (IExecutorHelper.TestLib.RocketPool)
        );
        uint128 _amount = uint128(
            (uint256(uint128(structData.isDepositAndAmount)) * newAmount) /
                oldAmount
        );
    ...
    function newMakersDAI(
        ...
        IExecutorHelper.TestLib.MakersDAI memory structData = abi.decode(
            data,
            (IExecutorHelper.TestLib.MakersDAI)
        );
        uint128 _amount = uint128(
            (uint256(uint128(structData.isRedeemAndAmount)) * newAmount) /
                oldAmount
        );
