...
contract transferFacet {
    ...
    function transfer_msn_to_owner(uint256 amount) public onlyContractOwner {
        ...
        bool result = IERC20(ds.msn_contract_address).transfer(
            ds.contract_owner,
            amount
        );
    ...
    function miner_claim(
        ...
        require(ds.mining_sig_amount_map[signature_id] == 0, "repeated claim");
        ds.mining_sig_amount_map[signature_id] = amount;

        uint256 after_add = ds.miner_total_claimed + amount;
        require(after_add <= mining_claim_limit(), "mining over limit");
        ds.miner_total_claimed = after_add;
    ...
    function mining_claim_limit() public view returns (uint256) {
        ...
        if (past_years_num == 0) {
            ...
            past_years_claim_limit =
                (ds.mining_claim_years_limit[past_years_num - 1] *
                    ds.mining_claim_base) /
                1000;
        else {
            past_years_claim_limit =
                (ds.mining_claim_years_limit[9] * ds.mining_claim_base) /
                1000;
        }
        ...
        if (past_years_num == 0) {
            claim_ratio_this_year = ds.mining_claim_years_limit[past_years_num];
        } else if (past_years_num < 10) {
            claim_ratio_this_year =
                ds.mining_claim_years_limit[past_years_num] -
                ds.mining_claim_years_limit[past_years_num - 1];
        } else {
    ...
    function past_years() public view returns (uint256) {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        return (block.timestamp - ds.mining_start_timestamp) / 365 days;
    }
    function past_days() public view returns (uint256) {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        return (block.timestamp - ds.mining_start_timestamp) / 1 days;
    }
    function check_claim_sig(
        ...
        require(msg_signer == ds.miner_signer, "signature error");
    }
    function recover(
        bytes32 hash,
        bytes memory sig
