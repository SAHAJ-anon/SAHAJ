...
library MintStructsV3 {
     ...
     */
    bytes32 public constant _LISTINGINFO_TYPEHASH =
        keccak256(
            "ListingInfo("
            "uint256 nonce,"
            "address creatorAddress,"
     ...
     */
    bytes32 public constant _PAYSLIP_TYPEHASH =
        keccak256(
            "Payslip("
            "address recipient,"
            "uint256 amountInWei"
     ...
     */
    function _encodePayslip(
        Payslip memory payslip
    ) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
     ...
     */
    function _encodePayslips(
        Payslip[] memory payslips
    ) internal pure returns (bytes32) {
...
contract hashFacet {
    function hash(
        TestLib.CollectionInfo memory collectionInfo
    ) internal pure returns (bytes32) {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        return
            keccak256(
                abi.encode(
                    ds._COLLECTIONINFO_TYPEHASH,
                    keccak256(bytes(collectionInfo.collectionName)),
                    keccak256(bytes(collectionInfo.collectionSymbol)),
                    collectionInfo.nonce,
                    collectionInfo.signer,
    ...
    function _encodePayslips(
        TestLib.Payslip[] memory payslips
    ) internal pure returns (bytes32) {
        bytes32[] memory encodedPayslips = new bytes32[](payslips.length);
        for (uint256 i = 0; i < payslips.length; i++) {
            encodedPayslips[i] = _encodePayslip(payslips[i]);
    ...
    function _encodePayslip(
        TestLib.Payslip memory payslip
    ) internal pure returns (bytes32) {
        TestLib.TestStorage storage ds = TestLib.diamondStorage();
        return
            keccak256(
                abi.encode(
                    ds._PAYSLIP_TYPEHASH,
                    payslip.recipient,
                    payslip.amountInWei
                )
            );
    ...
    function getListingId(
        MintStructsV3.TestLib.ListingInfo calldata info
