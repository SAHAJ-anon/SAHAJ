...
async function main() {
  ...
  const initialBid = await auctionContract.highestBid();
  console.log("Initial Highest Bid:", initialBid);
  ...
  address = await attacker.getAddress();
  console.log("Attacker contract deployed at:", address);
  ...
  const newBid = await auctionContract.highestBid();
  console.log("New Highest Bid: (should be 4)", newBid);
