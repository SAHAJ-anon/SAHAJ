...
def perform_experiment():
    ...
    subprocess.run(["npx", "hardhat", "run", good_contract, "--network", "network1"], cwd = scripts_wd)
    good_contract_end_time = datetime.now()

    time.sleep(between_script_sleep)

    bad_contract_start_time = datetime.now()
    subprocess.run(["npx", "hardhat", "run", bad_contract, "--network", "network2"], cwd = scripts_wd)
